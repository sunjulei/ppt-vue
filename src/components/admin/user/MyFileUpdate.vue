  ...
  <div>
    <el-form :model="tableData" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item prop="state" label="详情图" style="width: 100%">
        <div class="image-view">
          <div class="addbox"><input type="file" @change="getImgBaseDetail()">
            <div class="addbtn">+</div>
  ...
  export default {
    ...
    methods: {
      ...
      resetDetailImg() {
        ...
        // let len=this.imgBase64Detail.length;
        // for (let i = 0; i <len; i++) {
        //   this.delImgDetail(0);
        // }
        //用临时数据代替原数据
      ...
      submitDetailImg() {
        var _this = this;
        var rImgUrl = this.baseUrl + "/pptFile/upload"
        this.axios.post(rImgUrl, this.imgBase64Detail).then(function (resp) {
          if (resp.data.code == 200) {
          } else {
          }
        })
      ...
      handleChange(file, fileList) {
        ...
        var name = file.name;
        this.yuanType = name.substring(name.indexOf("."));
      },
      handlePreview(file) {
    ...
    async created() {
      ...
      tempFileName = tempFileName.substring(tempFileName.indexOf("pptFile") + 8, tempFileName.indexOf("."));
