  ...
  export default {
    ...
    methods: {
      ...
      handleEdit(index, row) {
        ...
        if (row.state === '通过' || row.state === '拒绝') {
          this.$confirm('当前文件审核状态为：' + row.state + '! 再次修改后，需要重新审核。是否继续?', '修改状态', {
      ...
      page(currentPage) {
        let _this = this;
        var url = this.baseUrl + "/pptFile/findAllByAuthor/"+this.$cookie.get('userck')+"/" + (currentPage - 1) + "/" + this.pageTotal
