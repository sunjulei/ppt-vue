  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(index, row) {
        let _this = this
        var url = this.baseUrl + "/fileDown/deleteById/" + row.id
      ...
      page(currentPage) {
        ...
        var url = this.baseUrl + "/fileDown/findAll/" + username + "/" + (currentPage - 1) + "/" + this.pageTotal
        ...
        this.axios.get(url).then(function (resp) {
          ...
          for (let i=0;i<resp.data.data.files.content.length;i++) {
            _this.tableData[i].downTime=resp.data.data.files.content[i].downTime
      ...
      deleteMultiplePpt() {
        let _this = this
        this.$confirm('此操作将永久删除选中的' + this.multipleSelection.length + '条下载记录，是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
          center: true
        }).then(() => {
          var url = this.baseUrl + "/fileDown/deleteAll"
